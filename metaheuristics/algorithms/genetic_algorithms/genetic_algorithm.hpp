...
class genetic_algorithm : public metaheuristic<G> {
	...
	protected:
		...
		crandom_generator<G,double> zero_one_rng;
		/// Discrete random generator for values in [0, @ref pop_size)
		drandom_generator<G,size_t> population_rng;
		 ...
		 */
		void generate_mutants(const problem<G> *p, size_t A, size_t B, population_set& next_gen, size_t& m);
		 ...
		 */
		void generate_crossovers(const problem<G> *p, population_set& next_gen, size_t& m);
		 ...
		 */
		void evaluate_individual(const problem<G> *p, individual& i) const;
		/**
		 ...
		 */
		void generate_mutant(const problem<G> *p, individual& i);
		 ...
		 * individuals.
		 */
		void crossover(const problem<G> *p, size_t i, size_t j, individual& son);
